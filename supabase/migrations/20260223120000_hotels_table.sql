...
-- Spatial index using standard btree (earthdistance extension not available)
CREATE INDEX IF NOT EXISTS idx_hotels_latitude ON hotels(latitude) WHERE latitude IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_hotels_longitude ON hotels(longitude) WHERE longitude IS NOT NULL;
